...
void genTextListMap();
void showBottomText(std::string s);
void showStatusText(std::string s);

std::vector<gviewer*> gviewer::vptr;
...
std::map<char, int> m_text_call_map;
			...
			for (const auto &t : mv_trajectory) {
				...
				for (int i = 0; i < t.size(); i++) {
					...
					if (i > t.size() - 20 && i <= t.size() - 5) {
						glColor4f(c_s[0] + (c_e[0] - c_s[0]) / 15.0 * n,
...
void gviewer::SetFrames(const std::vector<Frames> &vf)
{
	std::unique_lock<std::mutex> lock(m_mutex);
	mv_frames = vf;
}

void gviewer::AddNewFrame(const std::pair<Eigen::Matrix3d, Eigen::Vector3d> &f)
{
	std::unique_lock<std::mutex> lock(m_mutex);
	mv_frames.back().push_back(f);
}

void gviewer::SetPointCloud(const std::vector<VPointCloud> &vpc)
{
	std::unique_lock<std::mutex> lock(m_mutex);
	mv_pointCloud = vpc;
}

void gviewer::SetPointCloudSemantic(const std::vector<VPointCloud> &vpc)
{
	std::unique_lock<std::mutex> lock(m_mutex);
	mv_pointCloud_semantic = vpc;
}

void gviewer::SetInstances(const std::vector<VisualizedInstance> &vi)
{
	std::unique_lock<std::mutex> lock(m_mutex);
	mv_instances = vi;
}
...
void gviewer::AddNewPoint(const std::vector<Eigen::Vector3d> &pts)
{
	std::unique_lock<std::mutex> lock(m_mutex);
...
void gviewer::SetTrajectory(const std::vector<VPointCloud> &vt)
{
	std::unique_lock<std::mutex> lock(m_mutex);
	mv_trajectory = vt;
}
...
void gviewer::ScreenShot(const std::string &s)
{
	std::unique_lock<std::mutex> lock(m_mutex);
...
void gviewer::Show()
{
	if (t)
		return;
	mb_stop = false;
	t = new std::thread(&gviewer::Run, this);
	// t->detach();
}
...
void genTextListMap()
{
	std::map<char, uint64_t> letter_map = {
